...
#            set host ip and MAC address
            host = self.addHost('h%s' % (h + 1),ip='10.0.0.%s' % (h + 1),mac='00:11:22:33:44:0%s' % (h+1))
...
#    test connectivitiy between nodes
    ...
    for h in net.hosts:
        ...
        if int(index) <= minerNum:
            command = "xterm -title " + name + " -hold -e 'python3 Miner.py" + " "  + ip + " " + str(port) + " " + debugMode + " " + str(' '.join(workersIPs)) + "' &"
            #print(command)
