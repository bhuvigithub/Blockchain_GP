...
#            set host ip and MAC address
            if int(h)+1 < 10:
                macStr = '00:11:22:33:44:0' + str(h+1)
            else:
                macStr = '00:11:22:33:44:' + str(h+1)
            host = self.addHost('h%s' % (h + 1),ip='10.0.0.%s' % (h + 1),mac= macStr)
...
#    test connectivitiy between nodes
    ...
    for h in net.hosts:
        ...
        if int(index) <= minerNum:
            command = "xterm -title " + name + " -hold -e 'python3 Miner.py" + " "  + ip + " " + str(port) + " " + debugMode + " " +str(minerNum)+" " + str(' '.join(minersIPs)) + " "+str(' '.join(workersIPs)) + "' &"
            print(command)
            print(net.get('h%s'%(index)).cmd(command))
            
        elif minerNum < int(index) < hostNum:
            command = "xterm -title " + name +" -hold -e 'python3 Worker.py" + " " + ip + " " + str(port) + " " + debugMode +" "+ str(workerNum) +  " "+ str(' '.join(minersIPs)) + "' &"
            print(command)
            print(net.get('h%s'%(index)).cmd(command))
        else:
