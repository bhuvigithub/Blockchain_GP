...
class Miner:
    def __init__(self, host, port, debug, minersSize,worker_nodes, minerIPs):
        ...
        """for i in range(int(self.minersSize)):
            str = self.minerIPList[i]
            if str == self.host:
                self."""
        self.start()
    ...
    def verify_miners(self, processedBlock):
        ...
        print("maxxxxxxxxxxx" + str(maxResult))
        for i in range(len(l)):
            if l[i] == maxResult:
                print("Miner on" + str(self.compareList[i]['ip']) + " is right.")
            else:
                print("Miner on" + str(self.compareList[i]['ip']) + " is corrupt.")
                print("Its content: " + str(l[i]))
...
def createWorker(WorkerDataList, minerSize):
     print(sys.argv[5:-minerSize])
     for i in (sys.argv[5:-minerSize]):
        list = i.split('.')
        print(list)
...
def minerList(minerSize):
    minerList = []
    for i in range(1, int(minerSize) + 1):
        ip = '10.0.0.'+ str(i)
        minerList.append(ip)
...
if __name__ == '__main__': 
    ...
    minerIPList = minerList(sys.argv[4])
    WorkerDataList =[]
    createWorker(WorkerDataList, len(minerIPList))
    print(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], WorkerDataList)
    miner = Miner(sys.argv[1], sys.argv[2], False, sys.argv[4], WorkerDataList, minerIPList)
